...
``` ts filename="actions.ts" copy
export async function getSimilar({id, limit}: {id: string, limit: number}) {
  const headers = new Headers();
  headers.append("Authorization", `Bearer ${env.NONSTOP_TOKEN}`);
  const response = await fetch(
    `https://www.usenonstop.com/api/unstable/imoveis/similar?id=${id}&limit=${limit}`,
    { headers },
  );
